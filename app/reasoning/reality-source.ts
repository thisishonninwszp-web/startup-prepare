...
/**
 * 只有"针对现实地图里一个具体处境做推理"的工具才接这座桥：贝叶斯算的是一个具体处境下的
 * 信念概率，费米估的是一个具体处境下的数量级，重构换的是看待一个具体处境的角度——现实快照
 * 是它们推理的输入，不是装饰。
 *
 * 第一性原理和外部视角刻意不接入：第一性原理拆解的是一句抽象信念（"B2B销售周期必然很长"），
 * 跟某个具体现实处境没有必然关系，硬接快照只是挂个装饰字段，推理逻辑用不上它；外部视角输入
 * 的是计划文本本身，虽然现实处境能提供一点校准背景，但不接也完全能正常工作，价值不足以再引入
 * 一套 UI（源picker、隐藏字段、详情页卡片）。评估过后判定这不是遗漏，而是有意不做。
 */
