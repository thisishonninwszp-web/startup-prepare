...
export type SkillDef = {
   ...
   * 代价：点满它要付什么。
   *
   * 只写"通向哪里"是只有利没有弊，看完还是不知道值不值得。
   * 每一门手艺都收费 —— 有的收时间，有的收关系，
   * 有的收的是"你会变成唯一能干这个的人"。
   * 写不出真代价的就空着：空着比编一句强。
   */
  cost?: string;
...
/**
 * 每项技能的代价。数据大，和 milestones 一样单独放一张表。
 * 只写了内核、印记，加上几项代价特别反直觉的 —— 其余的空着。
 */
const COSTS: Record<string, string> = {
  systemcraft: "系统建起来之后，只有你知道它为什么这么搭。它越好用，越没人愿意去读它",
  duediligence: "查得越深越难开口说「不知道」。你会开始为自己的结论过度辩护",
  acceptance: "退回别人的东西是要还的。退多了，下次没人第一个找你看",
  framing: "换问题的人常常显得在找麻烦，尤其在一个已经动手的团队里",
  decompose: "拆得越清楚，越容易被当成那个「负责拆」的人，而不是负责决定的人",
  orgdesign: "画职责边界一定会动到人。你会在还没做出成绩之前先得罪人",
  managingup: "上面的人开始依赖你的判断之后，你的错会被放大到他的层级",
  revenue: "钱的路径一旦由你设计，跌了也算你的",
  crisis: "救过一次火，下一次还是找你。会救火的人很难被放去做长的事",
  integration: "拼起来的东西你只懂接缝，不懂零件。零件方改一次，你就得重来",
  transcode: "两边都觉得你是自己人的时候，你哪边的核心圈都不在",
  clone: "把自己写成规则，等于把自己的不可替代性交出去一部分",
  offline: "为断网做的准备，99% 的日子都是白做的",
  humanproof: "留证据是纯成本。没人出问题的时候，它看起来完全是多余的",
  compounding: "前半年几乎没有回报。大多数人是在第四个月放弃的",
  curation: "决定组织忘掉什么，就是替别人决定他们看不到什么",
  judgement: "自己判就得自己认。这是把「我只是执行」这条退路亲手拆掉",
  requestion: "翻掉问题会让已经做完的工作作废，包括你自己的",
  handover: "交出去之后，做成的是他，不是你",
  settingtone: "定了调，后面每一次跑偏都会回来找你",
  selfrunning: "东西自己跑起来之后，你就不再是它每天必须的人了",
  ownership: "签字的人吃全部下行。上行往往是团队的，下行一定是你的",
  dealmaking: "局做成了，几方都记得对方，未必记得你",
  unattended: "值班表上没有你之后，别人也不知道你曾经每晚在守",
  gravity: "有人来找你之后，你就很难再挑事情做了 —— 来的是什么就是什么",
  custody: "交给编队之后，你会慢慢失去对细节的手感",
  retention: "把组织的记忆整理好，最先被替代的往往是整理它的人",
  oncall: "你会变成唯一能救的人。这既是安全感，也是天花板",
  silence: "别人不知道你知道什么。缄默的代价一直是被低估",
  interrogate: "盘问久了，会开始怀疑所有看起来顺的东西，包括对的",
  coding: "写得动之后，任何问题都会先想成一个能写的东西",
  aiorchestration: "指挥得越顺，越难判断自己是不是还做得动这件事",
  trustbuilding: "信用要很久才攒起来，一次就能清零",
  keepingup: "维系是纯支出。断掉的关系不会来提醒你它断了",
  training: "身体的回报周期比任何事都长，而且停两周就退",
  jpbiz: "语言过关之后，你会被默认承担所有需要日语的事",
  spreadsheet: "表做得好，你会一直被叫去做表",
};
...
const MILESTONES: Record<string, SkillMilestone[]> = {
  ...
  record: [{ at: MILESTONE_TIERS[0], name: "写下来", test: "连着七天，每天睡前写一句今天发生了什么" }, { at: MILESTONE_TIERS[1], name: "写具体", test: "写的是「周三那次会」，不是「最近状态不好」" }, { at: MILESTONE_TIERS[2], name: "翻得到", test: "两周前的事，你能从记录里翻出来" }],
  search: [{ at: MILESTONE_TIERS[0], name: "找得到", test: "半小时里找到原始那份，不是别人转述的版本" }, { at: MILESTONE_TIERS[1], name: "换得动词", test: "第一次搜不到，换一组说法又搜到了" }, { at: MILESTONE_TIERS[2], name: "绕得开", test: "官方渠道查不到，你从别的地方绕到了" }],
  skim: [{ at: MILESTONE_TIERS[0], name: "读得快", test: "十分钟翻完一份长东西，能说出它在讲什么" }, { at: MILESTONE_TIERS[1], name: "挑得出", test: "五份摆在面前，你挑出唯一值得细看的那份" }, { at: MILESTONE_TIERS[2], name: "放得下", test: "读了两页发现没价值，你直接放下了" }],
  summarize: [{ at: MILESTONE_TIERS[0], name: "压得下", test: "一页的东西，你压成三句" }, { at: MILESTONE_TIERS[1], name: "不丢关键", test: "别人只看你写的三句，没做错判断" }, { at: MILESTONE_TIERS[2], name: "换得了口径", test: "同一件事，给老板和给同事你会写成两版" }],
  askbasic: [{ at: MILESTONE_TIERS[0], name: "问出口", test: "会上没听懂，你当场问了出来" }, { at: MILESTONE_TIERS[1], name: "不装懂", test: "在自己人面前承认过一次「我不懂」" }, { at: MILESTONE_TIERS[2], name: "问到点", test: "你问的那句，让别人也跟着明白了" }],
  paraphrase: [{ at: MILESTONE_TIERS[0], name: "说得回来", test: "对方讲完，你能把他的意思说一遍" }, { at: MILESTONE_TIERS[1], name: "对方点头", test: "他说「对，就是这个意思」" }, { at: MILESTONE_TIERS[2], name: "说得更清", test: "他说「你讲得比我清楚」" }],
  remembernames: [{ at: MILESTONE_TIERS[0], name: "记得住", test: "见过两次的人，你叫得出名字" }, { at: MILESTONE_TIERS[1], name: "记得住在意的", test: "你说得出他最在意什么" }, { at: MILESTONE_TIERS[2], name: "接得上", test: "隔半年再见，你接上了上次的话头" }],
  toolcraft: [{ at: MILESTONE_TIERS[0], name: "会用", test: "不碰鼠标，你把一件常做的事做完了" }, { at: MILESTONE_TIERS[1], name: "会配", test: "你把工具调成了顺手的样子" }, { at: MILESTONE_TIERS[2], name: "会换", test: "换一个没用过的工具，三天上手" }],
  arithmetic: [{ at: MILESTONE_TIERS[0], name: "估得出", test: "不打开表格，你口算出一个量级并说得出依据" }, { at: MILESTONE_TIERS[1], name: "单位不错", test: "百万和亿你没搞混过" }, { at: MILESTONE_TIERS[2], name: "看得出离谱", test: "一眼看出某个数字不可能对" }],
  spreadsheet: [{ at: MILESTONE_TIERS[0], name: "做得出", test: "一堆散数据，你整理成了一张能看的表" }, { at: MILESTONE_TIERS[1], name: "算得动", test: "用公式算的，不是一个个手填的" }, { at: MILESTONE_TIERS[2], name: "别人看得懂", test: "别人拿你的表直接就用了" }],
  formatting: [{ at: MILESTONE_TIERS[0], name: "有层次", test: "标题、段落、重点，一眼分得开" }, { at: MILESTONE_TIERS[1], name: "扫一眼有数", test: "别人扫一眼就知道哪里是重点" }, { at: MILESTONE_TIERS[2], name: "成模板", test: "有人拿你的格式去套自己的东西" }],
  punctual: [{ at: MILESTONE_TIERS[0], name: "不迟到", test: "连着四周，一次没迟到" }, { at: MILESTONE_TIERS[1], name: "提前说", test: "赶不上的时候你提前说了，不是事后解释" }, { at: MILESTONE_TIERS[2], name: "交期不飘", test: "答应的日子，连着三次没跳票" }],
  statusreport: [{ at: MILESTONE_TIERS[0], name: "主动说", test: "没人问，你自己报了一次进展" }, { at: MILESTONE_TIERS[1], name: "说得准", test: "你说的进度和实际对得上" }, { at: MILESTONE_TIERS[2], name: "先说坏的", test: "出问题那次，是你先开的口" }],
  backup: [{ at: MILESTONE_TIERS[0], name: "有备份", test: "重要的东西不止存在一个地方" }, { at: MILESTONE_TIERS[1], name: "恢复过", test: "真的从备份里捞回来过一次" }, { at: MILESTONE_TIERS[2], name: "自动做", test: "备份不靠你记得" }],
  shield: [{ at: MILESTONE_TIERS[0], name: "关得掉", test: "你关掉了一个一直在打断你的通知" }, { at: MILESTONE_TIERS[1], name: "拿得回", test: "一天里有一整段时间没被算法插过队" }, { at: MILESTONE_TIERS[2], name: "不反弹", test: "四周过去了，你没有偷偷装回来" }],
  trace: [{ at: MILESTONE_TIERS[0], name: "查得到", test: "一句转述，你追回了它最早是谁说的" }, { at: MILESTONE_TIERS[1], name: "看得出断点", test: "你指出它在哪一环被改了意思" }, { at: MILESTONE_TIERS[2], name: "追得动匿名", test: "没有署名，你也定位到了来源" }],
  ...
  listening: [{ at: MILESTONE_TIERS[0], name: "不打断", test: "一次对话里，你完整听完了他三段话" }, { at: MILESTONE_TIERS[1], name: "听得出没说的", test: "你说出了他绕开的那部分" }, { at: MILESTONE_TIERS[2], name: "听完能改", test: "听完之后，你改掉了自己原来的做法" }],
  observing: [{ at: MILESTONE_TIERS[0], name: "记得住", test: "事后你能白描出现场发生了什么" }, { at: MILESTONE_TIERS[1], name: "看得出不对劲", test: "你指出了一个和说法对不上的细节" }, { at: MILESTONE_TIERS[2], name: "看得出模式", test: "三次现场，你抽出了同一条规律" }],
  ...
  sleepcraft: [{ at: MILESTONE_TIERS[0], name: "记得住", test: "连着两周，你记下了每天睡多久" }, { at: MILESTONE_TIERS[1], name: "睡得够", test: "过半的晚上睡够了七小时" }, { at: MILESTONE_TIERS[2], name: "睡得稳", test: "作息波动在一小时以内" }],
