...
- predictions: id, idea_id, text, due_at, made_at, outcome(enum pending/hit/miss), resolved_at, note

其余模块（现状/顾客/复盘/梦想/推理/顾问团/知识库/公司/触达等）的表定义见
`supabase/migrations/`（按编号顺序执行）；`supabase/schema.sql` 是全新安装用的完整拼合版，
新增迁移时必须同步追加到该文件末尾。
